...
public class SpringGlobalMacroProvider extends VtlGlobalMacroProvider
{
	...
	@Override
	@Nonnull
	public Collection<VtlMacro> getGlobalMacros(@Nonnull final VelocityFile file)
	{
		...
		if(aPackage != null)
		{
			for(final PsiDirectory directory : aPackage.getDirectories())
			{
				...
				if(springVmFile instanceof VelocityFile)
				{
					return ((VelocityFile) springVmFile).getDefinedMacros();
				}
			}
		}
